...
/// How completely an RLGR decode consumed its input relative to its output.
///
/// Zero runs are implicit (they consume no input bits), so a run-length
/// mis-computation shifts every following coefficient in the output while the
/// bitstream stays formally valid. The tell-tale is the output filling up
/// while a non-trivial amount of input is still unread.
#[derive(Debug, Clone, Copy)]
pub struct RlgrConsumption {
    /// Bits left unread in the input when decoding stopped.
    pub input_bits_remaining: usize,
    /// Output slots left unfilled when decoding stopped (trailing zeros are
    /// normal: encoders may rely on implicit zero fill for the tail).
    pub output_remaining: usize,
}
...
pub fn decode(
    ...
    tile: &[u8],
    mut output: &mut [i16],
) -> Result<RlgrConsumption, RlgrError> {
    #![expect(
        clippy::as_conversions,
    ...
    let consumption = RlgrConsumption {
        input_bits_remaining: bits.len(),
        output_remaining: output.len(),
    };
