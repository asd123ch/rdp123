...
impl ClearCodecDecoder {
    ...
    ///
    /// **Alpha contract:** The residual, bands, raw, and RLEX layers are
    /// opaque BGR. NSCodec subregions carry straight alpha and are composed
    /// over the supplied base by [`Self::decode_with_base`].
    pub fn decode(&mut self, data: &[u8], width: u16, height: u16) -> DecodeResult<Vec<u8>> {
        self.decode_with_base(data, width, height, None)
    }

    /// Decode a ClearCodec bitmap stream, compositing onto `base`.
    ///
    /// The three ClearCodec layers do not necessarily cover the whole
    /// rectangle; per the reference implementation they composite directly
    /// onto the destination surface, leaving uncovered pixels untouched.
    /// `base` therefore should be the current surface content of the
    /// destination rectangle (`width * height * 4` BGRA bytes). Passing
    /// `None` composites over black, which erases whatever was underneath —
    /// only correct when the stream is known to cover the full rectangle.
    pub fn decode_with_base(
        ...
        // Handle glyph hit: return cached pixel data. The reference
        // implementation only requires the cached glyph to hold at least
        // width * height pixels (they are reinterpreted at the current
        // dimensions); exact dimension equality rejects real streams.
        if stream.is_glyph_hit() {
            ...
            let needed = pixel_count
                .checked_mul(4)
                .ok_or_else(|| invalid_field_err!("dimensions", "glyph pixel count overflow"))?;
            if entry.pixels.len() < needed {
                return Err(invalid_field_err!("glyphIndex", "cached glyph too small for region"));
            }
        ...
        // Decode composite payload. Non-glyph streams composite onto the
        // caller-provided surface content; glyph streams composite onto
        // zeros (the reference implementation decodes glyphs into their own
        // buffer, so glyph payloads always cover their full rectangle).
        let is_glyph_store = stream.flags & FLAG_GLYPH_INDEX != 0;
        let mut output = match base {
            Some(b) if !is_glyph_store && b.len() >= pixel_count * 4 => {
                b[..pixel_count * 4].to_vec()
            }
    ...
    fn decode_composite(
        ...
        // Parsed via the corrected reader (see parse_fixes): the upstream
        // SHORT_VBAR_CACHE_MISS bit layout is wrong and rejects real streams.
        if !composite.bands_data.is_empty() {
            let bands = parse_fixes::decode_bands_layer(composite.bands_data)?;
            for band in &bands {
    ...
    fn resolve_vbar(
        ...
        match vbar {
            VBar::CacheHit { index } => {
                // A hit on an empty slot must not fail the whole region: the
                // reference decoder fills the slot with dummy data and keeps
                // rendering ("If the cache was reset we need to fill in some
                // dummy data").
                match self.vbar_cache.get_vbar_for_height(*index, band_height) {
                    Some(full) => Ok(full),
                    None => {
                        tracing::warn!(index = *index, "ClearCodec V-bar cache hit on empty slot; filling dummy data");
                        let full = FullVBar {
                            pixels: vec![0u8; usize::from(band_height) * 3],
                        };
                        self.vbar_cache.put_vbar_at(*index, full.clone());
                        Ok(full)
                    }
                }
            }
            VBar::ShortCacheHit { index, y_on } => {
                // An empty short slot decodes as count = 0 in the reference
                // decoder, yielding a pure background column.
                let (pixel_count, pixels) = match self.vbar_cache.get_short_vbar(*index) {
                    Some(cached_short) => (cached_short.pixel_count, cached_short.pixels.clone()),
                    None => {
                        tracing::warn!(
                            index = *index,
                            "ClearCodec short V-bar cache hit on empty slot; using background"
                        );
                        (0, Vec::new())
                    }
                };
    ...
    fn decode_subcodec_region(
        ...
        match sub.codec_id {
            ...
            SubcodecId::Rlex => {
                // Parsed via the corrected reader (see parse_fixes): upstream
                // mis-handles single-entry palettes and loses suite pixels.
                let rlex = parse_fixes::decode_rlex(sub.bitmap_data)?;
                let w = usize::from(sub.width);
            ...
            SubcodecId::NsCodec => {
                ...
                if !self.nscodec_announced {
                    ...
                    for pixel in decoded.chunks_exact(4) {
                        ...
                        translucent += usize::from(alpha != 0 && alpha != 255);
                    }
                    tracing::debug!(
                        width = sub.width,
                        height = sub.height,
                        ...
                        alpha_max,
                        translucent,
                        "ClearCodec NSCodec subregions active"
                    );
                    self.nscodec_announced = true;
                }
                ...
                for row in 0..h {
                    ...
                    let src_row = &decoded[src_start..src_start + w * 4];
                    let dst_row = &mut output[dst_start..dst_start + w * 4];
                    for (dst, src) in dst_row.chunks_exact_mut(4).zip(src_row.chunks_exact(4)) {
                        source_over_bgra(dst, src);
                    }
                }
            }
        }
...
fn source_over_bgra(dst: &mut [u8], src: &[u8]) {
    ...
    for channel in 0..3 {
        let value = u32::from(src[channel]) * alpha + u32::from(dst[channel]) * inverse;
        dst[channel] = u8::try_from((value + 127) / 255).unwrap_or(255);
    }
    let output_alpha = alpha + (u32::from(dst[3]) * inverse + 127) / 255;
    dst[3] = u8::try_from(output_alpha).unwrap_or(255);
}
...
mod tests {
    ...
    #[test]
    fn nscodec_subcodec_decode() {
        let mut decoder = ClearCodecDecoder::new();
        let mut nscodec = Vec::new();
        for _ in 0..4 {
            nscodec.extend_from_slice(&2u32.to_le_bytes());
        }
        nscodec.extend_from_slice(&[1, 0, 0, 0]);
        nscodec.extend_from_slice(&[100, 100]); // Y
        nscodec.extend_from_slice(&[50, 0]); // Co
        nscodec.extend_from_slice(&[0, 50]); // Cg
        nscodec.extend_from_slice(&[255, 255]); // A

        let mut subcodec_data = Vec::new();
        subcodec_data.extend_from_slice(&0u16.to_le_bytes());
        subcodec_data.extend_from_slice(&0u16.to_le_bytes());
        subcodec_data.extend_from_slice(&2u16.to_le_bytes());
        subcodec_data.extend_from_slice(&1u16.to_le_bytes());
        subcodec_data.extend_from_slice(&u32::try_from(nscodec.len()).unwrap().to_le_bytes());
        subcodec_data.push(0x01); // SubcodecId::NsCodec
        subcodec_data.extend_from_slice(&nscodec);
        ...
        let pixels = decoder.decode(&stream, 2, 1).unwrap();
        assert_eq!(pixels, [50, 100, 150, 255, 50, 150, 50, 255]);
    }

    #[test]
    fn nscodec_subcodec_alpha_composites_over_base() {
        let mut decoder = ClearCodecDecoder::new();
        let mut nscodec = Vec::new();
        for _ in 0..4 {
            nscodec.extend_from_slice(&2u32.to_le_bytes());
        ...
        nscodec.extend_from_slice(&[1, 0, 0, 0]);
        nscodec.extend_from_slice(&[100, 100]); // Y
        nscodec.extend_from_slice(&[50, 50]); // Co
        nscodec.extend_from_slice(&[0, 0]); // Cg
        nscodec.extend_from_slice(&[0, 128]); // A
        ...
        let base = [10, 20, 30, 255, 10, 20, 30, 255];
        let pixels = decoder
            .decode_with_base(&stream, 2, 1, Some(&base))
            .unwrap();
        assert_eq!(&pixels[..4], &base[..4]);
        assert_eq!(&pixels[4..], &[30, 60, 90, 255]);
    }
